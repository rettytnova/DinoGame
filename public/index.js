...
function createSprites() {
  ...
  // itemImages = 포켓볼 아이템 정보를 담은 배열
  let imageIdx = 0;  
  const itemImages = items.data.map((item) => {
    const image = new Image();
    image.src = ITEM_IMAGE[imageIdx++];
    return {
      image,
      id: item.id,
...
function gameLoop(currentTime) {
  ...
  if (!gameover && !waitingToStart) {
    ...
    // 아이템
    itemController.update(gameSpeed, deltaTime, score.checkStage());
    // 달리기
